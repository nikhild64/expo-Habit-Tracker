/**
 * Gamification rules + helpers — port of src/lib/gamification/rules.ts.
 *
 * Everything in here is pure: deterministic level table, deterministic-seeded
 * daily-quest selection, achievement checker, evaluateQuests. No side effects.
 */
import type {
  Achievement,
  AchievementId,
...
import { isDoneToday } from './streak.util';
...
/**
 * Computes the coins earned for a single completion event.
 *
 * Split out from awardXP itself so unit tests can lock the coin math down
 * deterministically without spinning up a service.
 */
export function computeCoinAward(amount: number, event: XpEvent): number {
  let coinGain = COINS_COMPLETE_HABIT;
  if (event.allHabitsDone) coinGain += COINS_ALL_DONE_BONUS;
  const wasStreak7 = amount >= XP_STREAK_7; // crude — XP_STREAK_7 = 100 indicates milestone
